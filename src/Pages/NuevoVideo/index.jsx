...
export const NuevoVideo = () => {
    ...
    const validacao = (e) => {
        e.preventDefault();
        if(!titulo || !categoria || !imagem || !video || !descripcion){
            alert('Erro! completa todos los campos para continuar.')
        }else{
            const novoVideo = {
                titulo,
                categoria,
            ...
            const cadastrar = async () => {
                try{
                    const resposta = await axios.post('https://api-boe.onrender.com/videos', nuevoVideo);
                    if(respuesta){
                        navigate('/suceso');
                    }
                }catch(error){
    ...
    return(
        <>
            <FormularioAddVideo onSubmit={(e)=>{validacion(e)}}>
                   <div>
                        <h1>Crear Card</h1>
                   </div>
